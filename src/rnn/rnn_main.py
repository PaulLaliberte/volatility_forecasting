...
def preprocess_data(df, configs):
    ...
    if w_hours:
        ...
        if garch == 0:
            ...
            df["garch_vol"] = scaler.fit_transform(
                df["garch_vol"].values.reshape(-1, 1)
            )
...
def draw_plot(company_name, hyper_parameter, plot_data):
    ...
    ax.set_title(
        company_name + "'s MSE results w/ " + hyper_parameter,
    ...
    for d1 in data_per_company:
        ...
        for d2 in d1[1]:
            ...
            for key in d2[1].keys():
                ...
                try:
                    plot_data_list[hyper_parameter_name].append(
                        [model_name, d2[1][key]]
                    )
                except:
